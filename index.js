...
let IndexScheduledExecution = setInterval(function() {
    ...
    if (status == "complete") {
        if (localStorage && !localStorage.getItem("tip1")) {
            layer.confirm({
                title: "一些问题",
                message: "若您发现此页面出现了一些小问题，请先尝试清理浏览器缓存，看看问题是否仍然存在在然后决定再决定是否提交问题。<br>他们一般出现在页面多次自发性刷新后。<br>或者点击<button onclick='location.href = `/resource/page/debug.html`'>按钮</button>前往debug界面。",
                callback: {
